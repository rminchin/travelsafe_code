...
class NewConversationSearchState extends State<NewConversationSearch> {
  ...
  _openChatScreen(String friendUsername, String friendNickname) async {
    await DatabaseHelper.addConversation(widget.user.username, friendUsername, widget.user.nickname, friendNickname);
    Map<String,dynamic> user = await DatabaseHelper.getUserByUsernameFirebase(friendUsername);
  ...
  Future<List<Map<String,dynamic>>> findResults() async {
    ...
    List<String> friends = await DatabaseHelper.getFriendsListFirebase(widget.user.username);
    List<Map<String, dynamic>> conversations = await DatabaseHelper.getConversationsFirebase(widget.user.username);
    List<String> users = [];
    List<Map<String,dynamic>> newConversation = [];
    for(Map<String,dynamic> f in conversations){
      if(f['username1'] == widget.user.username && friends.contains(f['username2'])){
        users.add(f['username2']);
      } else if(f['username2'] == widget.user.username && friends.contains(f['username1'])){
  ...
  Widget _buildItem(int index) {
    ...
    return ListTile(
      ...
      trailing: ElevatedButton.icon(
          ...
          onPressed: () async{
              _openChatScreen(_results[indexToUse]['username'], _results[indexToUse]['nickname']);
          },
          style: ButtonStyle(
                  ...
                  children: [
                    ...
                    Flexible(
                      child: _results.isNotEmpty &&
                          ...
                          ? const Text('No friends in network to start a new conversation with')
                          : const Text(
...
class OpenChat extends StatefulWidget {
  final User user;
  final User user2;
  const OpenChat({Key? key, required this.user, required this.user2}) : super(key: key);
          ...
          children: [
            Expanded(
              child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Text("Chatting with " + widget.user2.nickname)
                  ]),
            ),
          ],
        ));
