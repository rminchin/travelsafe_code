...
class AddFriendState extends State<AddFriend> {
  ...
  void _backScreen() {
    Navigator.push<void>(
        ...
        MaterialPageRoute<void>(
            builder: (BuildContext context) => HomePage(user: widget.user, tab: 1)));
  }

  Future<void> addFriend(String username) async {
    await DatabaseHelper.sendRequestFirebase(widget.user.username, username, widget.user.nickname);
    Map<String, dynamic> u = await DatabaseHelper.getUserByUsernameFirebase(username);
    setState(() {
      _requests.add(u);
    });
  ...
  Future<void> findMatchingUsers() async {
    ...
    List<Map<String, dynamic>> requests = await DatabaseHelper.getRequestsFirebase(widget.user.username);
    List<Map<String, dynamic>> friends = await DatabaseHelper.getFriendsFirebase(widget.user.username);
    setState(() {
      _results = r;
      _requests = requests;
      _friends = friends;
  ...
  Widget _buildItem(int index) {
    ...
    return ListTile(
      ...
      trailing: ElevatedButton.icon(
        icon: Icon(
          friends ? Icons.people_rounded : requested ? Icons.check : Icons.arrow_right_alt_rounded,
          color: friends ? Colors.blueAccent : requested ? Colors.green : Colors.black,
        ),
            label: Text(friends ? 'Friends' : requested ? 'Pending' : 'Add'),
        onPressed: () async{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(centerTitle: true, title: const Text("TravelSafe"),leading: BackButton(
          color: Colors.white,
...
class ViewRequestsState extends State<ViewRequests> {
  ...
  Future<void> initializePreference() async {
    _results = await DatabaseHelper.getRequestsReceivedFirebase(widget.user.username);
  }
  ...
  Future<void> acceptRequest(String username, String nick) async {
    await DatabaseHelper.acceptFriendFirebase(username, widget.user.username, nick, widget.user.nickname);
    Map<String, dynamic> u = await DatabaseHelper.getUserByUsernameFirebase(username);
    setState(() {
      _results.remove(u);
    });
  ...
  Future<void> findMatchingUsers() async {
    List<Map<String, dynamic>> r = await DatabaseHelper.getRequestsReceivedFirebase(widget.user.username);
    setState(() {
      _results = r;
    });
  }
      ...
      trailing: SizedBox(
        ...
        child: Row(
            children: [
              GestureDetector(
              onTap: () async {
                await acceptRequest(_results[index]['from'], _results[index]['from_nickname']);
              },
...
class ManageNetworkState extends State<ManageNetwork> {
  ...
  Future<void> removeFriend(String username, String other) async {
    ...
    Map<String, dynamic> u = await DatabaseHelper.getUserByUsernameFirebase(other);
    setState(() {
      _friends.remove(u);
    });
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  Future<void> findMatchingUsers() async {
    List<Map<String, dynamic>> r = await DatabaseHelper.getFriendsFirebase(widget.user.username);
    setState(() {
      _friends = r;
    });
  }
  ...
  Widget _buildItem(int index) {
    ...
    int indexToUse = index ~/ 2;
    String user = _friends[indexToUse]['user1'] == widget.user.username ? _friends[indexToUse]['user2'] : _friends[indexToUse]['user1'];
    String nick = _friends[indexToUse]['user1_nickname'] == widget.user.nickname ? _friends[indexToUse]['user2_nickname'] : _friends[indexToUse]['user1_nickname'];
